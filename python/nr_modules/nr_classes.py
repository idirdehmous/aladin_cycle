...
class nr_maintenance(Family, nr_node):
  def __init__(self, suite_config):
    ...
    # ECMWF Time Critical suites have particular maintenance tasks
    if suite_config.mode == "tc" :
      self.add(
        Task("switch_sthost", Defstatus("complete"), 
             Edit(suite_config.jobs["localjob"])),
        Task("sync_tcwork", Defstatus("complete")),
        Task("establish_main_data", Defstatus("complete")),
        )
    else :
      self.add(
...
class nr_wait(Family, nr_node):
  def __init__(self, suite_config, i):
    ...
    if suite_config.has_assimilation and ( (not suite_config.has_loop) or i > 0 ) :
      ...
      # FIXME: don't wait in case of coldstart (or re-start, but that is harder)
      # FIXME: this trigger is WRONG in realtime loop if it refers to previous day
      trigger = "../../{0}/next_cycle/save_first_guess/{1} == complete OR :RUNDATE == :COLDSTART ".format(
                    prev_run, "%02d" % suite_config.cycle_inc)
      self += Task("wait_firstguess",
                   Edit(suite_config.jobs["localjob"]),
                   Trigger(trigger))
...
class nr_initialisation(Family, nr_node):
  def __init__(self, suite_config, i):
    ...
    # FIXME: when restarting a realtime suite, this will need to be reset manually...
    # probably more clear if this is added to the "wait" family
...
#        prev_run, "%02d" % suite_config.cycle_inc))
    ...
    if suite_config.has_alert :  # set the "timer"
...
#                        suite_config.suite_name, suite_config.next_run(i))),
...
class nr_lbc(Family):
  def __init__(self, suite_config, fclen):
    ...
    self.add_task("retrieve_lbc").add(
            Edit(WALLTIME="@WALLTIME_LBC:00:15:00@"), # set much higher if you are waiting for telecom files!
...
class nr_dfi(Task):
  def __init__(self, suite_config, tasktype="forecast"):
    Task.__init__(self, "DFI")
    self.add(Edit(ECF_FILES = suite_config.suite_path + "/scr/forecast",
                  NODE_SELECT = "@SELECT_DFI@",
                  WALLTIME = "@WALLTIME_DFI:0:15:00@"))

class nr_integration(Task):
  def __init__(self, suite_config, fclen, tasktype="forecast"):
    Task.__init__(self, "integration")
    self.add(Edit(ECF_FILES = suite_config.suite_path + "/scr/forecast",
                  NODE_SELECT = "@SELECT_FC@",
                  WALLTIME = "@WALLTIME_FC:0:30:00@"))
    if suite_config.has_dfi :
      self.add(Trigger("./DFI == complete"))
...
class nr_forecast(Family):
  def __init__(self, suite_config, fclen):
    ...
    if suite_config.has_dfi :
      self += nr_dfi(suite_config)

    self += nr_integration(suite_config, fclen)

    self += Task("monitor").add(
                  Edit(WALLTIME = "@WALLTIME_FC:0:30:00@"),
...
class nr_next_cycle(Family):
  def __init__(self, suite_config, fclen, next_run):
    ...
    if not suite_config.has_loop and suite_config.cycle_inc > 0 :
      self += Task("queue_next",
                 Trigger("../initialisation == complete && /{0}/cycle/{1} == complete"\
                     .format(suite_config.suite_name, next_run)),
    ...
    for hh in range(suite_config.cycle_inc, hhmax + 1, suite_config.cycle_inc):
      if suite_config.has_surfex and hh == suite_config.cycle_inc :
        self += Family("%02d" % hh,
                   ...
                   Edit(ECF_FILES=suite_config.suite_path + "/scr/next_cycle"),
                   Task("copy_Ts"),
...
class nr_postproc(Family):
  ...
  def __init__(self, suite_config, config):
    ...
    for hh in range(suite_config.forecast_length+1) :
  ...
  def __init__(self, suite_config):
    ...
    self.add(
      ...
      Trigger("./initialisation == complete && \
            ...
            (:ALERT_TIME >= /{0}:TIME && :ALERT_DD != /{0}:DD)  )".format(suite_config.suite_name)),
...
class nr_finish(Family):
  ...
  def __init__(self, suite_config, is_runcycle):
   ...
   # set family to "complete by default"
    if (not suite_config.has_loop) or ( int(suite_config.begindate) == 0 ) :
       self.add(Task("set_defstatus", Edit(suite_config.jobs["localjob"])))
## this task moved to lagged family
#    if suite_config.has_assimilation :
#      self.add(
